                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <table id="datatable-buttons" class="table table-striped table-bordered dt-responsive nowrap"
                            ...
                            <tr>
                                <th>ID</th>
                                <th>@lang('backend.payment-method'):</th>
                                <th>@lang('backend.name'):</th>
                                <th>@lang('backend.company'):</th>
                            ...
                            @foreach($appeals as $appeal)
                                <tr>
                                    <td>{{ $appeal->id }}</td>
                                    <td>{{ ($appeal->payment_type == "bankAccount") ? __('backend.pay-to-card') : __('backend.e-qaime')  }}</td>
                                    <td>{{ $appeal->admin()->id ?? '-' }}</td>
                                    <td>{{ \App\Models\Admin::find($appeal->admin_id)->company()->name ?? '-' }}</td>
                                    <td><a class="text-decoration-none text-black"
                                           href="tel:{{ $appeal->phone }}">{{ $appeal->phone }}</a></td>
                                    <td><a class="text-decoration-none text-black"
                                           href="mailto:{{ $appeal->email }}">{{ $appeal->email }}</a></td>
                                    <td>{{ $appeal->location }}</td>
                                    <td class="text-center">
                                        <a class="btn btn-danger"
                                           href="{{ route('backend.delAppeals',['id'=>$appeal->id]) }}">
