...
@section('title',__('backend.education'))
@section('styles')
                    ...
                    <div class="card-body">
                        <div class="col-12">
                            <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                                <h4 class="mb-sm-0">@lang('backend.education'):</h4>
                                <a href="{{ route('backend.education.create')}}" class="btn btn-primary mb-3"><i
                                        class="fas fa-plus"></i>
                        ...
                        <table id="datatable" class="table table-striped table-bordered dt-responsive nowrap"
                            ...
                            <tbody>
                            @foreach($educations as $city)
                                <tr>
                                    <td class="text-center">{{ $city->id }}</td>
                                    @foreach(active_langs() as $lang)
                                        <td class="text-center">{{ $city->translate($lang->code)->name }}</td>
                                    @endforeach
                                    <td>{{ date('d.m.Y H:i:s',strtotime($city->created_at))}}</td>
                                    <td class="text-center">
                                        <a class="btn btn-primary"
                                           href={{ route('backend.education.edit',['education'=>$city->id]) }}>
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        <a class="btn btn-danger"
                                           href="{{ route('backend.delEducation',['id'=>$city->id]) }}">
