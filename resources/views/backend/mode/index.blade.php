                    ...
                    <div class="card-body">
                        <div class="col-12">
                            <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                                <h4 class="mb-sm-0">@lang('backend.work-mode'):</h4>
                                <a href="{{ route('backend.modes.create')}}" class="btn btn-primary mb-3"><i
                                        class="fas fa-plus"></i>
                        ...
                        <table id="datatable" class="table table-striped table-bordered dt-responsive nowrap"
                            ...
                            @foreach($modes as $mode)
                                <tr>
                                    ...
                                    @foreach(active_langs() as $lang)
                                        <td class="text-center">{{ $mode->translate($lang->code)->name }}</td>
                                    @endforeach
                                    <td>{{ date('d.m.Y H:i:s',strtotime($mode->created_at))}}</td>
                                    <td class="text-center">
                                        <a class="btn btn-primary"
                                           href={{ route('backend.modes.edit',['mode'=>$mode->id]) }}>
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        <a class="btn btn-danger"
                                           href="{{ route('backend.delMode',['id'=>$mode->id]) }}">
