                    ...
                    <div class="card-body">
                        <div class="col-12">
                            <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                                <h4 class="mb-sm-0">@lang('menus.products'):</h4>
                                <a href="{{ route('backend.products.create')}}" class="btn btn-primary mb-3"><i
                                        class="fas fa-plus"></i>
                        ...
                        <table id="datatable" class="table table-striped table-bordered dt-responsive nowrap"
                            ...
                            @foreach($products as $product)
                                <tr>
                                    ...
                                    <td class="text-center"><img src="{{ asset($product->photo) }}" width="100"
                                                                 height="50"></td>
                                    <td class="text-center">{{ $product->alt }}</td>
                                    <td class="text-center">{{ $product->translate('az')->name }}</td>
                                    <td>{{ date('d.m.Y H:i:s',strtotime($product->created_at))}}</td>
                                    <td class="text-center">
                                        <a class="btn btn-primary"
                                           href={{ route('backend.products.edit',['product'=>$product->id]) }}>
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        <a class="btn btn-danger"
                                           href="{{ route('backend.delProduct',['id'=>$product->id]) }}">
