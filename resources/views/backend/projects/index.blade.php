                    ...
                    <div class="card-body">
                        <div class="col-12">
                            <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                                <h4 class="mb-sm-0">@lang('menus.Projects'):</h4>
                                <a href="{{ route('backend.projects.create')}}" class="btn btn-primary mb-3"><i
                                        class="fas fa-plus"></i>
                        ...
                        <table id="datatable-buttons" class="table table-striped table-bordered dt-responsive nowrap"
                            ...
                            @foreach($projects as $project)
                                <tr>
                                    ...
                                    <td class="text-center">{{ $project->translate('az')->title }}</td>
                                    <td>{{ date('d.m.Y H:i:s',strtotime($project->created_at))}}</td>
                                    <td class="text-center">
                                        <a href="{{ route('backend.projectStatus',['id'=>$project->id]) }}"
                                           title="@lang('backend.status')">
                                            <input type="checkbox" id="switch"
                                                   switch="primary" {{ $project->status == 1 ? 'checked' : '' }} />
                                            <label for="switch4"></label>
                                        </a>
                                    </td>
                                    <td class="text-center">
                                        <a class="btn btn-primary"
                                           href={{ route('backend.projects.edit',['project'=>$project->id]) }}>
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        <a class="btn btn-danger"
                                           href="{{ route('backend.delProject',['id'=>$project->id]) }}">
