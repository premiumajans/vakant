                    ...
                    <div class="card-body">
                        <div class="col-12">
                            <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                                <h4 class="mb-sm-0">@lang('menus.services'):</h4>
                                <a href="{{ route('backend.services.create')}}" class="btn btn-primary mb-3"><i
                                        class="fas fa-plus"></i>
                        ...
                        <table id="datatable" class="table table-striped table-bordered dt-responsive nowrap"
                            ...
                            @foreach($services as $service)
                                <tr>
                                    ...
                                    <td class="text-center"><img src="{{ asset($service->photo) }}" width="100"
                                                                 height="50"></td>
                                    <td class="text-center">{{ $service->translate('az')->name }}</td>
                                    <td>{{ date('d.m.Y H:i:s',strtotime($service->created_at))}}</td>
                                    <td>
                                        <a href="{{ route('backend.serviceStatus',['id'=>$service->id]) }}"
                                           title="@lang('backend.status')">
                                            <input type="checkbox" id="switch"
                                                   switch="primary" {{ $service->status == 1 ? 'checked' : '' }} />
                                            <label for="switch4"></label>
                                        </a></td>
                                    <td class="text-center">
                                        <a class="btn btn-primary"
                                           href={{ route('backend.services.edit',['service'=>$service->id]) }}>
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        <a class="btn btn-danger"
                                           href="{{ route('backend.delService',['id'=>$service->id]) }}">
