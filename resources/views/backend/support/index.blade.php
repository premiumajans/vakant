...
@section('title',__('menus.technical-support'))
@section('styles')
                    ...
                    <div class="card-body">
                        <div class="col-12">
                            <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                                <h4 class="mb-sm-0">@lang('menus.technical-support'):</h4>
                                <a href="{{ route('backend.support.create') }}" class="btn btn-primary mb-3"><i
                        ...
                        <table id="datatable-buttons" class="table table-striped table-bordered dt-responsive nowrap"
                            ...
                            <tbody>
                            @foreach($supports as $statistic)
                                <tr>
                                    <td class="text-center">{{ $statistic->id }}</td>
                                    <td class="text-center">{{ $statistic->translate('az')->description }}</td>
                                    <td>{{ date('d.m.Y H:i:s',strtotime($statistic->created_at))}}</td>
                                    <td class="text-center">
                                        <a class="btn btn-primary"
                                           href={{ route('backend.support.edit',['support'=>$statistic->id]) }}>
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        <a class="btn btn-danger"
                                           href="{{ route('backend.delSupport',['id'=>$statistic->id]) }}">
